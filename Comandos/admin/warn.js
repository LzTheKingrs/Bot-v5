...
module.exports = {
  ...
  run: async (client, interaction, args) => {
    ...
    if (member.permissions.has(Discord.PermissionFlagsBits.ManageGuild)) {return interaction.reply({
      content: `**⛔ | ${interaction.user}, Esse Usuario não pode, pois ele tem Administrador!`,
      ephemeral: true, })
    }
    if (membro.id === interaction.guild.members.cache.get(`1112418569641263176`)) {
      return interaction.reply('PORQUE VOCÊ QUER DAR AVISO NO MEU DONO??????')
    }
    ...
    if (member.roles.cache.has(cargo3)) {
      member.ban({ reason: 'Banido por ultrapassar o limite de advertências.' })
        .then(() => {
          ...
          const warnban = new Discord.EmbedBuilder()
        ...
        .then(() => {
          member.roles.add(cargo3)
            .then(() => {
              ...
              const warn3 = new Discord.EmbedBuilder()
